...
def task1():
  ...
  for i in range(len(numbers)):
    ...
    for i in range(len(boards)):
      ...
      if win is not None:
        inner_sum = [sum(dig for dig, stamp in row if stamp != 1) for row in boards[i]]
...
def task2():
  ...
  unmarked_row_sums = [sum(dig for dig, stamp in row if stamp != 1) for row in boards[completed_board_indexes[-1][0]]]
